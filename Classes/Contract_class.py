...
'''  # Реквизиты сторон физлицо
    ...
    def declension_of_the_word(self):
        ...
        for n in rs:
            list3.append(n)
        if middlename_deligate in list2 or firstname_deligate in list1 or lastname_deligate in list3:
            female_gender = True
    ...
    def to_word(self):
        ...
        document = DocxTemplate(f"{config.CONTRACT_PATH}dogowor_template_1.docx")
        ...
        document.save(f"{config.CONTRACT_PATH}contract{self.number}_{self.Obshaya_summa_dogovora}.docx")

    def to_word_measurements_contract(self):
        from docxtpl import DocxTemplate
        document = DocxTemplate(f"{config.CONTRACT_PATH}dogowor_template_measure.docx")
        context = {'number': self.number,
                   'townobject': self.townobject,
                   'data': self.date,
                   'customername': self.customername,
        ...
        document.render(context)
        document.save(f"{config.CONTRACT_PATH}contract_measure{self.number}_{self.Obshaya_summa_dogovora}.docx")

    def to_word_legal(self):
        from docxtpl import DocxTemplate
        document = DocxTemplate(f"{config.CONTRACT_PATH}dogowor_template_legal.docx")
        context = {'number': self.number,
                   'townobject': self.townobject,
                   'data': self.date,
                   'customername': self.customername,
        ...
        document.render(context)
        document.save(f"{self.path}contract_legal_{self.number}_{self.pefix}.docx")


# def Schetchic_Saver(schetchik):
#     with open(config.SCHETCHIK_PATH_CONTRACT, "wb") as f:
#         pickle.dump(schetchik, f, protocol=pickle.HIGHEST_PROTOCOL)
#     with open(config.SCHETCHIK_PATH_CONTRACT, 'rb') as f:
#         loaded_data = pickle.load(f)  # Загружаем экземпляр класса
...
if __name__ == '__main__':
    ...
    print(new_contract.First_pay_word)
    print(new_contract)
    doc = new_contract.text_legal_entity
    new_contract.to_word_legal()
